...
In early theories of language acquisition, children’s environment was assumed to “trigger” specific innate developmental schemas (e.g., Gibson & Wexler, 1994). 
Driven by influential observations of the tight relationship between environmental input and children’s learning (e.g., Huttenlocher et al., 1991; Hart & Risley, 1995), more recent models have emphasized the accumulation of specific linguistic experiences in driving learning and generalization. 
...
Although this work is exciting, these models have not yet been used to make direct predictions about learning in individual children (Bergelson, 2020). 
...
These absolute units give the potential for a model to make powerfully general quantitative predictions, which could be tested across different situations and populations (Dailey & Bergelson, in press). 
...
Since seminal work by Hart & Risley (1996), the connection between children’s language input and the growth of vocabulary has been a topic of intense interest, and numerous studies have reported associations between these variables [e.g., @hoff2003specificity; @weisleder2013]. 
...
These studies use regression models to predict which words are easier or more difficult, typically averaging across children (e.g., Goodman, Dale, & Li, 2008; Braginsky et al., 2018). 
...
Such models can also be used to assess the value of other proposals for learning mechanisms in the literature (e.g., Fourtassi et al., 2020). 
...
This model outlines an approach that is very similar to Item-Response Theory (IRT; Embretson & Reise, 2000), an influential and popular psychometric modeling framework for understanding the performance of test-takers (in our case language learners whose vocabulary is being assessed) as they are assessed with a particular set of test items (the particular words we ask about). 
...
With this mapping into absolute units, this kind of model then can provide a quantitative linking hypothesis between measurements of input and learning. 
...
As covariates we included the lexical class of each word (function words, adjective, noun, verb, or other), and the estimated numbers of tokens per word a child is likely to hear each month (range: 0.2 - 19,286 tokens), based on estimates from the CHILDES corpus of child-directed speech (MacWhinney, 2000). 
...
Predicted acquisition curves from Model 1 are shown in Figure 3, broken down by age, lexical class, prevalence (left), as well as for specific 

then outline a series of candidate “standard” models to test down the road--some of which will require new data sources in order to differentiate.


```{r, out.width="1.0\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Predicted acquisition curves from the fitted English “standard model” as a function of lexical class and the number of expected tokens per month (left), and for a sample of the 680 CDI words (right)."), echo=FALSE}
