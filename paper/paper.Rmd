...
[^1]: Note that "standard" here is not meant to imply that the model is a normative prescription, but rather that the model is an accepted starting point for a description of early language learning.
...
Accumulator models, which we focus on here, assume that linguistic experiences with words accumulate in separate registers (depicted as "buckets" in Figure 2), and that those registers that exceed a particular threshold are learned. 
...
They typically show a strong association between word frequency and the age at which children typically acquire particular words, especially for object labels. 
...
IRT models jointly infer both the difficulty of specific test items and the accuracy of specific test takers, recovering these latent parameters from the observed data. 
...
Using the well-known IRT framework to formalize theoretical questions and assess them via model comparison is a benefit over more ad-hoc frameworks. 
